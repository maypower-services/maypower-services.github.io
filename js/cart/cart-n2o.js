...
var active      = false,
    ...
    host        = "mws.systems", //null == transition.host ? "mws.systems" /*window.location.hostname*/ : transition.host,
    port        = null == transition.port ? window.location.port : transition.port;

function N2O_start() {
    ws = new bullet(protocol + host + (port==""||protocol=="wss://"?"":":"+port) + "/ws" + querystring);
...
function query_string() {
    ...
    if (qs.indexOf("project=") == -1)
        qs += (window.location.search == "" ? "?" : "&") + "project=" + window.project_id;
    return qs;
}
